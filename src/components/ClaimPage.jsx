...
const ClaimPage = ({ code }) => {
  ...
  const checkAndClaim = async (e) => {
    ...
    try {
      ...
      // Only set the transaction hash if txHash is defined
      if (txHash) {
        setTransactionHash(txHash);
      } else {
        // Handle case where txHash is undefined (optional)
        setTransactionHash(null); // or handle it differently as needed
      }
    } catch (error) {
      console.error(error);
      // Handle error if needed
      setTransactionHash(null); // Reset hash in case of error
    }
  };

  const { isLoading: isConfirming, isSuccess: isConfirmed } = useWaitForTransactionReceipt({
    hash: transactionHash || undefined, // Ensure hash is either string or undefined
  });
